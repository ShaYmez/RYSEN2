        ...
        for section in config.sections():
            if section == 'GLOBAL':
                ...
                CONFIG['ALIASES'].update({
                    ...
                    'SUBSCRIBER_URL': config.get(section, 'SUBSCRIBER_URL', fallback='https://radioid.net/static/users.json'),
                    'TGID_URL': config.get(section, 'TGID_URL', fallback='https://api.freestar.network/v1/talkgroup_ids.json'),
                    ...
                    'SERVER_ID_URL': config.get(section, 'SERVER_ID_URL', fallback='https://api.freestar.network/v1/server_ids.csv'),
                    'SERVER_ID_FILE': config.get(section, 'SERVER_ID_FILE', fallback='server_ids.tsv'),
                    'CHECKSUM_URL': config.get(section, 'CHECKSUM_URL', fallback='https://api.freestar.network/v1/file_checksums.json'),
